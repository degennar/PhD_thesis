...
\begin{equation}
    ...
    \label{eq:ki-hamiltonian-k-space}
\end{equation}
%
...
\begin{equation}
    \begin{split}
        ...
        &= \sum_{\bq} \int d\br' \ \frac{1}{|\br - \br'|} e^{i \bq \cdot \br'} \sum_{\bg} \rho_{\bq+\bg}^{n \bR} e^{i \bg \cdot \br'} \\
        & = \sum_{\bq} \sum_{\bg} \int d\br' \frac{1}{|\br -\br'|} e^{i(\bq+\bg) \cdot \br'} \rho_{\bq+\bg}^{n \bR} e^{i(\bq+\bg) \cdot (\br'-\br)} e^{-i(\bq+\bg) \cdot (\br' -\br)} \\
        &= \sum_{\bq} \sum_{\bg} \left\{ \int d\br' \ \frac{1}{|\br -\br'|} e^{-i(\bq+\bg) \cdot (\br -\br')} \right\}\rho_{\bq+\bg}^{n \bR} e^{-i(\bq+\bg) \cdot \br} \\
        %& = \sum_{\bq} \left\{ \sum_{\bg} e^{-i\bg \cdot \br} \frac{4\pi}{|\bq+\bg|^2} \rho_{\bq+\bg}^{\bR n}  \right\} e^{-i\bq \cdot \br} \nonumber \\
        %& = \sum_{\bq}  e^{-i\bq \cdot \br}\ \Delta V_{\rm H,\bq}^{\bR n}(\br)
    \end{split}
\end{equation}
